...
/*   Updated: 2025/11/26 01:30:42 by x03phy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void convert_elf_header32_endian( Elf32_Ehdr *elf_header, int little )
{
	if ( NEED_SWAP( little ) )
	{
		...
		elf_header->e_entry = ft_bswap32( elf_header->e_entry );
		elf_header->e_phoff = ft_bswap32( elf_header->e_phoff );
		elf_header->e_shoff = ft_bswap32( elf_header->e_shoff );
												...
												int little )
{
	if ( NEED_SWAP( little ) )
	{
		for ( uint16_t i = 0; i < count; i += 1 )
		{
			...
			sections[i].sh_flags = ft_bswap32( sections[i].sh_flags );
			sections[i].sh_addr = ft_bswap32( sections[i].sh_addr );
			sections[i].sh_offset = ft_bswap32( sections[i].sh_offset );
			sections[i].sh_size = ft_bswap32( sections[i].sh_size );
			sections[i].sh_link = ft_bswap32( sections[i].sh_link );
			sections[i].sh_info = ft_bswap32( sections[i].sh_info );
			sections[i].sh_addralign = ft_bswap32( sections[i].sh_addralign );
			sections[i].sh_entsize = ft_bswap32( sections[i].sh_entsize );
		}
	}
}
...
void convert_elf_symbol32_endian( Elf32_Sym *sym, int little )
{
	if ( NEED_SWAP( little ) )
	{
		...
		sym->st_value = ft_bswap32( sym->st_value );
		sym->st_size = ft_bswap32( sym->st_size );
